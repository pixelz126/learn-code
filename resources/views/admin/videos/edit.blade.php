                        ...
                        <form method="post" action="{{ route('videos.update', $video) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('title') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="title" id="input-title" class="form-control form-control-alternative{{ $errors->has('title') ? ' is-invalid' : '' }}" placeholder="{{ __('Enter video title') }}" value="{{ $video->title }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('link') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="link" id="input-link" class="form-control form-control-alternative{{ $errors->has('link') ? ' is-invalid' : '' }}" placeholder="{{ __('https://www.example.com/watch?v=wuvHO5lGVFE') }}" value="{{ $video->link }}" required>
                              ...
                              <div class="form-group{{ $errors->has('course_id') ? ' has-danger' : '' }}">
                                    ...
                                    <select name="course_id" required class="form-control">
                                        ...
                                        <option <?php if($video->course->id == $course->id) echo "selected"; ?> value="{{$course->id}}"> {{$course->title}} </option>
                                        @endforeach
                                    </select>
